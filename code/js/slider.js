...
function initSlider() {
  ...
  rangeSlider.noUiSlider.on(
    ...
    function (values, handle) {
      ...
      rangeSliderValueElement.innerHTML =
        "Exploring text entries from   " +
        start +
        "   to   " +
        end +
        " (" +
        (scale * 100).toFixed(2) +
        "%) . Word Cloud Generation make take a few seconds.";
      update();
    }
  );
  ...
  rangeSlider.noUiSlider.on(
    ...
    function (values, handle) {
      ...
      scale = (end - start) / dcount;
      let scalar_min =
        rangeSlider.noUiSlider.options.range.min;
      scalar_max = rangeSlider.noUiSlider.options.range.max;
      scale_range = scalar_max - scalar_min;
      if (
        end - start <
          sliderRescaleRange.min * scale_range ||
        end - start > sliderRescaleRange.max * scale_range
      ) {
        setSliderRange(2 * start - end, 2 * end - start);
      } else if (
        start - scalar_min <
        sliderRescaleRange.left * scale_range
      ) {
        let target = 2 * start - end;
        setSliderRange(target, target + scale_range);
      } else if (
        scalar_max - end <
        sliderRescaleRange.right * scale_range
      ) {
